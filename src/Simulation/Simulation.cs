	...
	public class Simulation
	{
		...
		public Stage[] RunSimulation(LogMsg _log)
		{
			...
			while (currentStage >= 0)
			{
				...
				// Store the list of smart tanks that drain in this stage
				for (int i = 0; i < allParts.Count; ++i) {
					if (allParts[i].smartTank != null) {
						if (allParts[i].smartTank.DrainStage == currentStage) {
							stage.drainedTanks.Add(allParts[i].smartTank);
						}
					}
				}
		...
		private void ActivateStage()
		{
			...
			for (int i = 0; i < allParts.Count; ++i)
			{
				...
				if (partSim.decoupledInStage >= currentStage)
				{
					...
					if (partSim?.smartTank != null) {
						// Tell the SmartTankPart which stage it drains in
						partSim.smartTank.DrainStage = partSim.decoupledInStage + 1;
					}
				}
			}
